...
export default function BulkVCardCreator() {
  ...
  function load(value: string, name = "contacts.csv") { setText(value); setFileName(name); }
  function handleFile(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => load(String(reader.result ?? ""), file.name); reader.readAsText(file); event.target.value = ""; }
