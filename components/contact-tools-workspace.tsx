...
const configs: Record<ContactToolMode, { title: string; eyebrow: string; description: string; multiple?: boolean }> = {
  ...
  "split-group": { title: "Split VCF by group", eyebrow: "MERGE & SPLIT", description: "Create one VCF per category or organisation group." },
  ...
  "extract-companies": { title: "Extract companies", eyebrow: "EXTRACT & TRANSFORM", description: "Export organisations, departments, and titles." },
  ...
  sort: { title: "Sort VCF contacts", eyebrow: "MERGE & SPLIT", description: "Sort contacts by name, organisation, email, phone, or original order." },
  ...
  count: { title: "Count VCF contacts", eyebrow: "EXTRACT & TRANSFORM", description: "Summarise contact count and useful fields without exporting values." },
  ...
  "group-company": { title: "Group contacts by company", eyebrow: "MERGE & SPLIT", description: "Summarise contact groups by organisation." },
  "group-domain": { title: "Group contacts by email domain", eyebrow: "MERGE & SPLIT", description: "Summarise contact groups by email domain." },
  ...
  "file-analyzer": { title: "VCF file analyser", eyebrow: "OPEN & EDIT", description: "Summarise versions, fields, warnings, and file structure." },
  ...
  "normalize-organizations": { title: "Normalize organisations", eyebrow: "CLEAN & REPAIR", description: "Trim and collapse repeated whitespace in organisation fields." },
...
const defaultCleaner: CleanerOptions = { trimWhitespace: true, normalizeEmails: false, normalizePhones: false, removePhotos: false, removeEmpty: false, countryCode: "+234" };
...
export default function ContactToolsWorkspace({ mode }: { mode: ContactToolMode }) {
  ...
  const issues = useMemo(() => parseResult ? validateContacts(contacts, parseResult) : [], [contacts, parseResult]);
  ...
  async function processFiles(files: FileList | File[]) {
    ...
    try { applyLoaded(await Promise.all((config.multiple ? selectedFiles : selectedFiles.slice(0, 1)).map(readFile))); }
  ...
  function splitDownload() {
    ...
    else if (mode === "split-group") { const groups = new Map<string, Contact[]>(); contacts.forEach((contact) => { const keys = contact.categories.length ? contact.categories : [contact.organisation || "ungrouped"]; keys.forEach((key) => groups.set(key, [...(groups.get(key) ?? []), contact])); }); groups.forEach((group, key) => { const safe = key.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "ungrouped"; files[`${safe}.vcf`] = new TextEncoder().encode(serializeVCards(group, { version: "3.0" })); }); }
  ...
  function removeDuplicates() { const next = removeDuplicateGroups(contacts, groups); setContacts(next); setGroups(findDuplicateGroups(next)); downloadVcf(next, "deduplicated"); }
  function mergeGroup(group: DuplicateGroup) { const next = mergeContactGroup(contacts, group.ids); setContacts(next); setGroups(findDuplicateGroups(next)); setStatus("Merged a duplicate group"); }
  ...
  function reset() { setContacts([]); setParseResult(null); setRawText(""); setFileName(""); setFileNames([]); setGroups([]); setCleanedPreview(null); setFilterQuery(""); setStatus("Choose a VCF file to begin"); }
  ...
  return <div className="contact-tools-card"><div className="conversion-toolbar"><span className="file-icon" aria-hidden="true">VCF</span><div><strong>{fileName || config.title}</strong><small aria-live="polite">{status}{fileNames.length > 1 ? ` · ${fileNames.length} source files` : ""}</small></div>{fileName && <><button className="secondary-button" onClick={reset}>Open another</button>{actionButton}</>}</div>{!fileName ? <div className="conversion-dropzone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); processFiles(event.dataTransfer.files); }}><div className="drop-icon" aria-hidden="true">↥</div><h2>Choose {config.multiple ? "VCF files" : "a VCF file"}</h2><p>{config.description} Files stay in your browser.</p><div className="drop-actions"><button className="primary-button" onClick={() => inputRef.current?.click()}>Choose VCF {config.multiple ? "files" : "file"} <span>⌘</span></button><button className="text-button" onClick={loadSample}>Try a sample</button></div><input ref={inputRef} aria-label={`Choose VCF ${config.multiple ? "files" : "file"}`} type="file" accept=".vcf,.vcard,text/vcard" multiple={config.multiple} onChange={handleFile} hidden /><small>Local processing · no contact data upload</small></div> : <>
    ...
    {mode === "validator" && <section className="operation-section"><div className="operation-heading"><div><p className="eyebrow">VALIDATION RESULTS</p><h2>{issues.length ? `${issues.length} issue${issues.length === 1 ? "" : "s"} to review` : "No issues found"}</h2></div></div><div className="issue-list">{issues.slice(0, 30).map((issue, index) => <div key={`${issue.code}-${index}`}><span className={`issue-severity issue-${issue.severity}`}>{issue.severity}</span><span>{issue.message}</span></div>)}</div></section>}
    ...
    {mode !== "dedupe" && mode !== "cleaner" && mode !== "normalize-phones" && mode !== "remove-photos" && mode !== "validator" && mode !== "repair" && !isExtractionMode && !utilityModes && <section className="operation-section"><p className="mapping-note">{contacts.length.toLocaleString()} contacts from {fileNames.length || 1} source file{fileNames.length === 1 ? "" : "s"}. The merged output preserves unknown fields.</p></section>}
  </>}</div>;
}
