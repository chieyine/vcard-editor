...
export default function PdfWorkspace() {
  ...
  return <div className="conversion-card"><div className="conversion-toolbar"><span className="file-icon" aria-hidden="true">PDF</span><div><strong>{fileName || "VCF to PDF"}</strong><small aria-live="polite">{status}</small></div>{contacts.length > 0 && <button className="primary-button compact" onClick={createPdf} disabled={busy}>{busy ? "Creating…" : "Download PDF"} <span>↓</span></button>}</div>{!contacts.length ? <div className="conversion-dropzone"><div className="drop-icon" aria-hidden="true">↥</div><h2>Create a real PDF contact directory</h2><p>The PDF is generated entirely in this browser. The contact file is never uploaded.</p><button className="primary-button" onClick={() => inputRef.current?.click()}>Choose VCF</button><input ref={inputRef} type="file" accept=".vcf,.vcard,text/vcard" onChange={handleFile} hidden aria-label="Choose a VCF file" /></div> : <section className="preview-section"><div className="preview-heading"><div><p className="eyebrow">PDF PREVIEW</p><h2>{contacts.length.toLocaleString()} contacts</h2></div><button className="secondary-button" onClick={() => { setContacts([]); setFileName(""); }}>Open another</button></div><div className="contact-preview-grid">{contacts.slice(0, 6).map((contact) => <article key={contact.id}><strong>{contact.formattedName || "Unnamed contact"}</strong><span>{contact.organisation || contact.emails[0] || contact.phones[0] || "No additional details"}</span></article>)}</div><p className="mapping-note">The print-ready PDF embeds local Noto Sans subsets for Latin, Cyrillic, Greek, Devanagari, and Vietnamese names. Nothing is uploaded.</p></section>}</div>;
}
