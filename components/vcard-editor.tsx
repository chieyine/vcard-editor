...
function AddressFields({ contact, update }: { contact: Contact; update: (patch: Partial<Contact>) => void }) {
  const raw = contact.address ?? "";
  const parts = raw.includes(";") ? raw.split(";") : ["", "", raw];
  const compose = (index: number, value: string) => {
    const nextParts = [...parts];
    while (nextParts.length <= index) nextParts.push("");
    nextParts[index] = value;
    update({ address: nextParts.join(";") });
  };
  return <fieldset className="address-fields wide"><legend>Address</legend><div className="form-grid"><label>Street<input value={parts[2] ?? ""} onChange={(event) => compose(2, event.target.value)} /></label><label>City<input value={parts[3] ?? ""} onChange={(event) => compose(3, event.target.value)} /></label><label>Region / state<input value={parts[4] ?? ""} onChange={(event) => compose(4, event.target.value)} /></label><label>Postal code<input value={parts[5] ?? ""} onChange={(event) => compose(5, event.target.value)} /></label><label>Country<input value={parts[6] ?? ""} onChange={(event) => compose(6, event.target.value)} /></label></div></fieldset>;
}

function AdvancedContactFields({ contact, update }: { contact: Contact; update: (patch: Partial<Contact>) => void }) {
  return <section className="advanced-fields" aria-label="Additional contact fields"><p className="eyebrow">ADDITIONAL FIELDS</p><div className="form-grid"><label>Nickname<input value={contact.nickname ?? ""} onChange={(event) => update({ nickname: event.target.value })} /></label><label>Department<input value={contact.department ?? ""} onChange={(event) => update({ department: event.target.value })} /></label><label>Role<input value={contact.role ?? ""} onChange={(event) => update({ role: event.target.value })} /></label><label>Birthday<input type="date" value={contact.birthday ?? ""} onChange={(event) => update({ birthday: event.target.value })} /></label><label className="wide">Website<input value={contact.url ?? ""} onChange={(event) => update({ url: event.target.value })} placeholder="https://…" /></label><AddressFields contact={contact} update={update} /></div></section>;
}
...
export default function VCardEditor() {
  ...
  function bulkFindReplace() {
    if (!findText) { setStatus("Enter the text to find first"); return; }
    let affected = 0;
    const swap = (value: string) => value.split(findText).join(replaceText);
    const next = contacts.map((contact) => {
      const updated: Contact = { ...contact, firstName: swap(contact.firstName), lastName: swap(contact.lastName), formattedName: swap(contact.formattedName), organisation: swap(contact.organisation), title: swap(contact.title), note: swap(contact.note), nickname: contact.nickname ? swap(contact.nickname) : contact.nickname, department: contact.department ? swap(contact.department) : contact.department, role: contact.role ? swap(contact.role) : contact.role, url: contact.url ? swap(contact.url) : contact.url, address: contact.address ? swap(contact.address) : contact.address, phones: contact.phones.map(swap), emails: contact.emails.map(swap), categories: contact.categories.map(swap) };
      if (JSON.stringify(updated) !== JSON.stringify(contact)) affected += 1;
      return updated;
    });
    if (!affected) { setStatus(`No contacts contain “${findText}”`); return; }
    commitContacts(next, `Replaced ${affected.toLocaleString()} contact${affected === 1 ? "" : "s"} containing “${findText}”`);
  }

  function bulkRemove(field: "photo" | "note" | "address") {    const affected = contacts.filter((contact) => field === "photo" ? contact.photo : field === "note" ? contact.note : contact.address).length;
  ...
  useEffect(() => {
    const handleKeydown = (event: KeyboardEvent) => {
      ...
      } else if (key === "z" && !typing && past.length) {
        event.preventDefault();
  ...
  return <div className="editor-card">
    ...
    {!contacts.length ? <div className="dropzone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const files = Array.from(event.dataTransfer.files); if (files.length === 1) readFile(files[0]); else if (files.length > 1) { const invalid = files.map((file) => guardFile(file, [".vcf", ".vcard"])).find((result) => !result.ok); if (invalid && !invalid.ok) setStatus(invalid.message); else Promise.all(files.map(async (file) => decodeVCardBytes(await file.arrayBuffer()).text)).then((texts) => parseWithWorker(new TextEncoder().encode(texts.join("\r\n")).buffer, `${files.length}-files.vcf`)); } }}><div className="drop-icon" aria-hidden="true">↥</div><h2>Open contact files</h2><p>Drop one or several <strong>.vcf</strong> or <strong>.vcard</strong> files here, or choose them from your device.</p><div className="drop-actions"><button className="primary-button" onClick={() => inputRef.current?.click()}>Choose VCF files</button><button className="text-button" onClick={loadSample}>Try a sample file</button><button className="text-button" onClick={() => applyParsed({ contacts: [blankContact(0)], warnings: [], issues: [], versions: { "3.0": 1 }, lineEnding: "CRLF", encoding: "UTF-8", totalLines: 0 }, "new-contact.vcf", "browser")}>Create blank contact</button></div><input ref={inputRef} aria-label="Choose VCF files" type="file" multiple accept=".vcf,.vcard,text/vcard" onChange={handleFile} hidden /><small>Processed locally in your browser · nothing is uploaded</small>{status.includes("Parsing") || status.includes("Inspecting") ? <button className="text-button" onClick={cancelParsing}>Cancel processing</button> : null}</div> : <>
