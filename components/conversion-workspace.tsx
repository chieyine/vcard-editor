...
export default function ConversionWorkspace({ mode }: { mode: ConversionMode }) {
  ...
  function applyTabular(headersToUse: string[], rowsToUse: TabularRow[], nextWarnings: string[], sourceName: string, nextSheetName = "") {
    setHeaders(headersToUse); setRows(rowsToUse); setMapping(suggestMapping(headersToUse)); setPreset("generic"); setWarnings(nextWarnings); setFileName(sourceName); setSheetName(nextSheetName); setContacts([]); setStatus(`${rowsToUse.length.toLocaleString()} rows ready`);
  }
    ...
    reader.onload = async () => {
      ...
      try {
        if (config.inputKind === "vcf") {
          ...
          setContacts(parsed.contacts); setRows(nextRows); setHeaders(nextHeaders); setWarnings(parsed.warnings); setMapping({}); setPreset("generic"); setFileName(file.name); setStatus(`${parsed.contacts.length.toLocaleString()} contacts ready`); setSheetName("");
  ...
  function download() {
    if (!rows.length && !contacts.length) return;
      ...
      const outputRows = contactsToRows(contacts);
  ...
  function applyPreset(value: "generic" | "google" | "outlook") {
    ...
    headers.forEach((header) => {
      const normalised = header.toLowerCase().replace(/[_.-]+/g, " ");
      if (value === "google") {
        if (/given name|first name/.test(normalised)) next[header] = "firstName";
        if (/family name|last name/.test(normalised)) next[header] = "lastName";
        if (/phone.*value|phone 1/.test(normalised)) next[header] = "phone";
        if (/e mail.*value|email 1/.test(normalised)) next[header] = "email";
        if (/organization name|company/.test(normalised)) next[header] = "organisation";
        if (/notes/.test(normalised)) next[header] = "note";
        if (/labels|group/.test(normalised)) next[header] = "categories";
      } else {
        if (/^first name$/.test(normalised)) next[header] = "firstName";
        if (/^last name$/.test(normalised)) next[header] = "lastName";
        if (/mobile phone|business phone|home phone/.test(normalised)) next[header] = "phone";
        if (/e mail address|email/.test(normalised)) next[header] = "email";
        if (/company/.test(normalised)) next[header] = "organisation";
        if (/job title/.test(normalised)) next[header] = "title";
        if (/notes/.test(normalised)) next[header] = "note";
      }
    });
    setMapping(next);
  ...
  function reset() { setFileName(""); setContacts([]); setRows([]); setHeaders([]); setMapping({}); setPreset("generic"); setWarnings([]); setStatus("Choose a file to begin"); setSheetName(""); }
